...
namespace angel {
	...
	bool ODMmap::LoadODM() {
		...
		#define CHECK_OFF( xxx ) {angel::Log << angel::aeLog::debug <<boost::format( " off = %8x size = %8x")% off % (xxx) <<angel::aeLog::endl;off += (xxx);if( off > (int)datasize || off < 0 ) return false;}	
	...
	void	ODMmap::AddTerrainToScene() {/*
		...
		if( version == 8 ) {
			...
			switch( mastertile ) {
				case 0: 
					tilename = "language/dtile.bin";
					break;
				case 1: 
					tilename = "language/dtile2.bin";
					break;
				case 2: 
					tilename = "language/dtile3.bin";
					break;
				default: 
					tilename = "language/dtile.bin";
